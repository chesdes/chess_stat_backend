...
async def get_last_game_analyze(
    ...
    try:
        return await analysis_service.get_last_game_analysis(site, username, index, request=request)
...
async def save_last_game_analyze(
    ...
    try:
        return await analysis_service.save_last_game_analysis(site, username, index, payload, request=request)
    except GameNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except PlayerNotFoundError as exc:
        raise HTTPException(status_code=404, detail="Player not found") from exc
...
async def get_pgn_analyze_classifications(request: Request, payload: AnalyzeAndPgnPayload):
    """Calculate move classifications from a PGN and precomputed Stockfish results."""
    try:
        return await analysis_service.analyze_pgn(payload, request=request)
    except AnalysisCapacityError as exc:
        raise HTTPException(status_code=429, detail=str(exc)) from exc
    except UpstreamError:
        raise
